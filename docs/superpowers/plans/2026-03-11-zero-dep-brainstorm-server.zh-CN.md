...
**目标：** 用单一零依赖的 `server.js`（仅使用 Node 内置模块）替换 brainstorm 服务器随附的 node_modules。

**架构：** 单个文件，包含 WebSocket 协议（RFC 6455 文本帧）、HTTP 服务器（`http` 模块）与文件监听（`fs.watch`）。以模块方式被 require 时导出协议函数以便单元测试。
...
- **不改动：** `skills/brainstorming/scripts/helper.js`、`skills/brainstorming/scripts/frame-template.html`、`skills/brainstorming/scripts/stop-server.sh`
...
- [ ] **步骤 1：创建带 OPCODES 常量与 computeAcceptKey 的 server.js**
...
客户端帧总是被掩码。返回 `{ opcode, payload, bytesConsumed }`；不完整时返回 `null`。遇到未掩码帧时抛出异常。
...
- [ ] **步骤 2：添加 WAITING_PAGE、在模块作用域加载模板以及辅助函数**

在模块作用域加载 `frameTemplate` 与 `helperInjection`，以便 `wrapInFrame` 与 `handleRequest` 可访问。它们只从 `__dirname`（即 scripts 目录）读取文件，无论该模块是被 require 还是直接运行，这都是有效的。
...
文件监听逻辑内联在 `startServer`（步骤 6）中，以使 watcher 生命周期与服务器生命周期保持一致，并按 spec 要求包含 `error` 处理器。

- [ ] **步骤 6：添加 startServer 函数与条件式 main**

`frameTemplate` 与 `helperInjection` 已在模块作用域（步骤 2）。`startServer` 仅创建屏幕目录、启动 HTTP 服务器与 watcher，并打印启动信息。
...
测试目录已有一个以 `ws` 为依赖的 `package.json`。如有需要先安装，然后运行测试。
...
- [ ] **步骤 2：移除 node_modules 的 gitignore 例外**
...
预期：打印端口为 9876 的 `server-started` JSON

- [ ] **步骤 2：在浏览器中打开 http://localhost:9876**

预期：显示等待页，内容为 "Waiting for Claude to push a screen..."
...
预期：浏览器重新加载并显示被框架模板包裹的 "Hello from smoke test"

- [ ] **步骤 4：验证 WebSocket 工作正常 — 检查浏览器控制台**

打开浏览器开发者工具。WebSocket 连接应显示为已连接（控制台无错误）。框架模板的状态指示器应显示 "Connected"。

- [ ] **步骤 5：用 Ctrl-C 停止服务器并清理**
