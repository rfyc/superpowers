...
**目标：** 让 `using-git-worktrees`、`finishing-a-development-branch` 及相关技能在 Codex App 的沙箱化 worktree 环境中正常工作，同时不破坏现有行为。

**架构：** 在两个技能的开头进行只读环境检测（`git-dir` 对比 `git-common-dir`）。若已处于链接的 worktree 中，则跳过创建。若处于游离 HEAD（detached HEAD）上，则输出交接（handoff）载荷，而非 4 选项菜单。沙箱回退机制在创建 worktree 时捕获权限错误。
...
### 任务 1：为 `using-git-worktrees` 添加步骤 0
...
完整阅读 `skills/using-git-worktrees/SKILL.md`。确定准确的插入点：在 "Announce at start" 行（第 14 行）之后、"## Directory Selection Process"（第 16 行）之前。
...
再次阅读该文件。确认：
- 步骤 0 出现在 Overview 与 Directory Selection Process 之间
...
### 任务 3：为 `finishing-a-development-branch` 添加步骤 1.5
...
完整阅读 `skills/finishing-a-development-branch/SKILL.md`。确定插入点：在 "**If tests pass:** Continue to Step 2."（第 38 行）之后、"### Step 2: Determine Base Branch"（第 40 行）之前。

- [ ] **步骤 2：插入步骤 1.5 小节**

在步骤 1 与步骤 2 之间插入以下内容：
...
再次阅读该文件。确认：
- 步骤 1.5 出现在步骤 1 与步骤 2 之间
- 步骤 2-5 未变
- 路径 A 交接包含提交 SHA 与数据丢失警告
- 路径 B 与 C 正常进入步骤 2
...
### 任务 4：为 `finishing-a-development-branch` 添加步骤 5 清理守卫

**文件：**
- 修改：`skills/finishing-a-development-branch/SKILL.md`（步骤 5：Cleanup Worktree —— 按小节标题查找，任务 3 之后行号已偏移）

- [ ] **步骤 1：阅读当前步骤 5 小节**

在 `skills/finishing-a-development-branch/SKILL.md` 中找到 "### Step 5: Cleanup Worktree" 小节（任务 3 插入后行号已偏移）。当前步骤 5 为：
...
注意：原文说"For Options 1, 2, 4"，但 Quick Reference 表与 Common Mistakes 小节说的是"Options 1 & 4 only。"本修改使步骤 5 与这些小节保持一致。

- [ ] **步骤 3：验证替换**

阅读步骤 5。确认：
- 清理守卫（重新检测）出现在最前
- 对非外部托管的 worktree 保留了现有移除逻辑
...
### 任务 5：更新 `subagent-driven-development` 与 `executing-plans` 中的 Integration 行
...
阅读 `skills/subagent-driven-development/SKILL.md` 第 268 行与 `skills/executing-plans/SKILL.md` 第 68 行。确认两者均为 "Ensures isolated workspace (creates one or verifies existing)"。
...
应恰好显示 6 个文件变更（5 个技能文件 + 1 个测试文件）。无其他文件被修改。
...
注意：这些测试需要带 `--dangerously-skip-permissions` 的 Claude Code。若不可用，应记录说明回归测试需手动运行。
