...
> **面向 agent 执行者：** 必需的子技能：使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实施本计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**目标：** 让 superpowers 在可用时优先使用宿主 harness 的原生 worktree 系统，不可用时回退到手动 git worktree，并修复三个已知的收尾（finishing）bug。

**架构：** 重写两个技能文件（`using-git-worktrees`、`finishing-a-development-branch`），三个文件做一行式集成更新（`executing-plans`、`subagent-driven-development`、`writing-plans`）。核心变更是添加检测（`GIT_DIR != GIT_COMMON`）以及"原生工具优先"的创建路径。这些是 markdown 技能说明文件，而非应用代码——"测试"是使用 testing-skills-with-subagents TDD 框架的 agent 行为测试。

**技术栈：** Markdown（技能文件）、bash（测试脚本）、Claude Code CLI（无头测试使用 `claude -p`）

**Spec：** `docs/superpowers/specs/2026-04-06-worktree-rototill-design.md`

---

### 任务 1：门禁（GATE）— 对步骤 1a（原生工具优先）的 TDD 验证

步骤 1a 是整个设计的关键假设。如果 agent 不优先使用原生 worktree 工具而非 `git worktree add`，则 spec 失败。必须最先验证这一点，在触碰任何技能文件之前。

**文件：**
- 创建：`tests/claude-code/test-worktree-native-preference.sh`
- 阅读：`skills/using-git-worktrees/SKILL.md`（当前版本，用于 RED 基线）
- 阅读：`tests/claude-code/test-helpers.sh`（用于 `run_claude`、`assert_contains` 等）
- 阅读：`skills/writing-skills/testing-skills-with-subagents.md`（TDD 框架）

**本任务是一道门禁。** 若 GREEN 阶段在 2 次 REFACTOR 迭代后仍失败，则停止。不要进入任务 2。返回报告——创建方法需要重新设计。

- [ ] **步骤 1：编写 RED 基线测试脚本**

创建测试脚本，该脚本将分别在"无更新技能文本"与"有更新技能文本"两种情况下运行场景。RED 阶段针对当前技能（尚无步骤 1a）运行。
...
预期：`[RED CONFIRMED] Agent did NOT use EnterWorktree` — 由于当前技能没有原生工具优先设置，agent 使用 `git worktree add`。

逐字记录 agent 的确切输出以及任何合理化解释。这是技能必须修复的基线失败。

- [ ] **步骤 3：若 RED 得到确认，继续。编写步骤 1a 技能文本。**

创建技能的一个临时测试版本，仅包含步骤 1a 的添加内容（最小改动以隔离变量）。将该小节添加到技能创建说明的顶部、现有目录选择流程之前：
...
若失败：逐字记录 agent 的确切输出与合理化解释。这是一个 REFACTOR 信号——步骤 1a 的文本需要修订。最多尝试 2 次 REFACTOR 迭代。若 2 次迭代后仍失败，停止并回报。

- [ ] **步骤 5：运行 PRESSURE 阶段 — 确认 agent 在压力下抗拒回退**
...
若失败：逐字记录合理化解释。向步骤 1a 文本添加明确的对抗条目（例如 Red Flag 条目："当你的平台提供原生 worktree 工具时，绝不使用 git worktree add"）。重新运行。
...
对创建技能进行全面重写。完全替换现有文件。

**文件：**
- 修改：`skills/using-git-worktrees/SKILL.md`（全面重写，219 行 → 约 210 行）

**依赖：** 任务 1 GREEN 通过。

- [ ] **步骤 1：编写完整的新 SKILL.md**

将 `skills/using-git-worktrees/SKILL.md` 的整个内容替换为：
...
预期：约 200-220 行。扫描是否有任何 markdown 格式问题。
...
对收尾技能进行全面重写。添加环境检测，修复三个 bug，添加基于来源（provenance）的清理。

**文件：**
- 修改：`skills/finishing-a-development-branch/SKILL.md`（全面重写，201 行 → 约 220 行）

- [ ] **步骤 1：编写完整的新 SKILL.md**

将 `skills/finishing-a-development-branch/SKILL.md` 的整个内容替换为：
...
对三个引用 `using-git-worktrees` 的文件做一行式修改。
...
验证完整重写后的技能可以协同工作。运行现有测试套件并做手动验证。

**文件：**
- 阅读：`tests/claude-code/run-skill-tests.sh`
- 阅读：`skills/using-git-worktrees/SKILL.md`（验证最终状态）
- 阅读：`skills/finishing-a-development-branch/SKILL.md`（验证最终状态）

- [ ] **步骤 1：运行现有测试套件**

运行：`cd tests/claude-code && bash run-skill-tests.sh`

预期：所有现有测试通过。若任何测试失败，调查——集成变更（任务 4）可能破坏了某个内容断言。

- [ ] **步骤 2：重新运行步骤 1a GREEN 测试**

运行：`cd tests/claude-code && bash test-worktree-native-preference.sh green`

预期：PASS——使用最终技能文本（而非任务 1 中最小化的步骤 1a 添加）时，agent 仍使用 EnterWorktree。

- [ ] **步骤 3：手动验证 — 从头到尾阅读两个重写后的技能**

完整阅读 `skills/using-git-worktrees/SKILL.md` 与 `skills/finishing-a-development-branch/SKILL.md`。检查：

1. 无对旧行为的引用（硬编码的 `CLAUDE.md`、交互式目录提示、"REQUIRED" 语言）
2. 每个文件内部步骤编号一致
3. Quick Reference 表与正文一致
4. Integration 小节交叉引用正确
5. 无 markdown 格式问题
