...
**目标：** 通过原生 JavaScript 插件为 OpenCode.ai 提供完整的 superpowers 支持，该插件与现有的 Codex 实现共享核心功能。

**架构：** 将通用的技能发现/解析逻辑提取到 `lib/skills-core.js`，重构 Codex 以使用它，然后使用 OpenCode 的原生插件 API（自定义工具和会话钩子）构建 OpenCode 插件。
...
在文件顶部现有 requires 之后（大约第 6 行处），添加：
...
查找并替换所有调用，将 `extractFrontmatter(` 改为 `skillsCore.extractFrontmatter(`
...
找到关于受支持平台的章节（在文件中搜索 "Codex"），并在其后添加：
...
- [ ] 创建包含所有核心函数的 `lib/skills-core.js`
- [ ] 重构 `.codex/superpowers-codex` 以使用共享核心
- [ ] Codex 命令仍然可用（find-skills、use-skill、bootstrap）
- [ ] 创建带工具和钩子的 `.opencode/plugin/superpowers.js`
- [ ] 创建安装指南
