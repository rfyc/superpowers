...
在分派 task reviewer subagent 时使用此模板。Reviewer 通读一次任务的 diff，并返回两个裁决：规范符合性和代码质量。

**目的：** 验证一个任务的实现符合其需求（不多也不少），并且构建良好（干净、有测试、可维护）。
...
**占位符：**
- `[MODEL]` — 必填：按 SKILL.md 模型选择选定的 reviewer 模型
- `[BRIEF_FILE]` — 必填：任务简报文件（`scripts/task-brief PLAN N` 打印路径；implementer 从同一文件工作）
- `[GLOBAL_CONSTRAINTS]` — 从计划的全局约束一节或规范中逐字复制的约束性需求：精确值、格式，以及组件之间明确陈述的关系（不是流程规则 —— 那些已在本模板中）
- `[REPORT_FILE]` — 必填：implementer 写入其详细报告的文件
- `[BASE_SHA]` — 本任务之前的提交
- `[HEAD_SHA]` — 当前提交
- `[DIFF_FILE]` — 必填：controller 写入审查包的路径（`scripts/review-package PLAN_FILE BASE HEAD` 打印其写出的唯一路径；该包绝不进入 controller 的上下文）

**Reviewer 返回：** 规范符合性裁决（✅/❌/⚠️）、优点、发现（Critical/Important/Minor）、任务质量裁决
